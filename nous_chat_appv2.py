...
def setup_fonts():
    try:
        ...
        try:
            # Font dosyasını geçici bir yere kaydet ve yükle
            temp_dir = tempfile.gettempdir()
            font_path = os.path.join(temp_dir, "DejaVuSans.ttf")
            
            # DejaVu Sans TrueType font'u base64 formatında (kısaltılmış)
            font_data = "AAEAAAATAQAABAAwRFNJR54SRB0AAqC0AAAdYE9TLzJLa3QVAAABIAAAAGBWY21lbMVBklsAAALM..." 
            
            with open(font_path, "wb") as font_file:
                font_file.write(base64.b64decode(font_data))
            
            # Font dosyasını PDF oluşturucuya kaydet
...
def local_css():
    # Farklı temalar için CSS - sadece ana içerik alanını etkiliyor
    if theme == "dark":
        main_bg = "#282c34"  # Koyu tema için sağ taraf arka plan rengi
        text_color = "#E0E0E0"  # Koyu tema için metin rengi
        chat_bg_user = "#36404a"  # Koyu temada kullanıcı mesajı arka planı
        chat_bg_ai = "#2d3748"  # Koyu temada AI mesajı arka planı
    else:  # light tema
        main_bg = "#FFFFFF"  # Açık tema için sağ taraf arka plan rengi
        text_color = "#333333"  # Açık tema için metin rengi  
        chat_bg_user = "#E6F7FF"  # Açık temada kullanıcı mesajı arka planı
        chat_bg_ai = "#F0F0F0"  # Açık temada AI mesajı arka planı
...
def create_pdf(chat_history, conversation_name, language):
    ...
    styles.add(ParagraphStyle(name='Sender', 
...
with col2:
    ...
    # Güncellenmiş model listesi - portal.nousresearch.com sitesindeki gerçek modellerle
